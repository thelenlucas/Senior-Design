...
namespace woodworks::domain {
    ...
    {
        return "INSERT INTO logs (species, length, diameter, quality, drying, location, notes) "
               "VALUES (?, ?, ?, ?, ?, ?, ?);";
    }

    inline QString Log::updateSQL()
    {
        return "UPDATE logs SET species = ?, length = ?, diameter = ?, quality = ?, drying = ?, location = ?, notes = ? "
               "WHERE id = ?;";
    }
